 ...
 **************************************************************************************/
package org.linear.activemq.layer.accident;
...
{
    private static Log log = LogFactory.getLog(Accident.class);
    ...
    public Accident() throws Exception
    {
        ...
        InputStream propertiesIS = Accident.class.getClassLoader().getResourceAsStream(Constants.CONFIG_FILENAME);
        ...
        int port = Integer.parseInt(properties.getProperty(Constants.MGMT_RMI_PORT_ACCIDENTS_LAYER));
        LocateRegistry.createRegistry(port);

        // Obtain MBean servera
        log.info("Obtaining JMX server and connector");
        MBeanServer mbs = MBeanServerFactory.createMBeanServer();
        String jmxServiceURL = properties.getProperty(Constants.MGMT_SERVICE_URL_ACCIDENTS_LAYER);
        ...
        String jmsurl = properties.getProperty(Constants.JMS_PROVIDER_URL_ACCIDENTS_LAYER);
        ...
        String destination = properties.getProperty(Constants.JMS_INCOMING_DESTINATION_ACCIDENTS_LAYER);
        ...
        AccidentListener listeners[] = new AccidentListener[numListeners];
        for (int i = 0; i < numListeners; i++)
        {
            listeners[i] = new AccidentListener(engine.getEPRuntime());
