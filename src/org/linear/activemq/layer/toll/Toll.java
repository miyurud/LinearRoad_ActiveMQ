...
public class Toll
{
    ...
    public Toll() throws Exception
    {
        log.info("Loading properties");
        Properties properties = new Properties();
        InputStream propertiesIS = Toll.class.getClassLoader().getResourceAsStream(Constants.CONFIG_FILENAME);
        ...
        int port = Integer.parseInt(properties.getProperty(Constants.MGMT_RMI_PORT_TOLL_LAYER));
        LocateRegistry.createRegistry(port);

        // Obtain MBean servera
        log.info("Obtaining JMX server and connector");
        MBeanServer mbs = MBeanServerFactory.createMBeanServer();
        String jmxServiceURL = properties.getProperty(Constants.MGMT_SERVICE_URL_TOLL_LAYER);
        ...
        String jmsurl = properties.getProperty(Constants.JMS_PROVIDER_URL_TOLL_LAYER);
        ...
        String destination = properties.getProperty(Constants.JMS_INCOMING_DESTINATION_TOLL_LAYER);
        ...
        TollEventListener listeners[] = new TollEventListener[numListeners];
        
        for (int i = 0; i < numListeners; i++)
        {
            listeners[i] = new TollEventListener(engine.getEPRuntime());
